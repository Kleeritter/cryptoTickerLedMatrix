...
asset assets[] = {
    ...
    //ETH
    {true, "ETH",
     "https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies="+String(CURRENCY_CODE),
      0x03}
    };
      ...
      if (httpCode > 0) {
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
         ...
         // Calculate the price, change and percentage change
          ...
          if(assets[currentAsset].isCrypto){
            pricebtc = payloadbtc.substring(payloadbtc.indexOf(String(CURRENCY_CODE))+3,payloadbtc.indexOf(String(CURRENCY_CODE))+13).toFloat();;
